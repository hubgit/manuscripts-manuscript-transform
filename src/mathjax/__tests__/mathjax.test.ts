...
import { convertMathMLToSVG, convertTeXToMathML, convertTeXToSVG } from '../'
...
describe('mathjax', () => {
  ...
  it('generate inline mathml', async () => {
    const tex = '\\sqrt{2}-z_{foo}=\\sum{x}'
    const result = await convertTeXToMathML(tex, false)
    expect(result).toMatchSnapshot()
  })

  it('generate display mathml', async () => {
    const tex = '\\sqrt{2}-z_{foo}=\\sum{x}'
    const result = await convertTeXToMathML(tex, true)
    expect(result).toMatchSnapshot()
  })
